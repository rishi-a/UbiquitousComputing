...
const int PWM_pin = 3; // Arduino101 LED on pin13 does not have PWM capabilities

Gauge gauge(&dwgs);
VSlider vslider('a', &dwgs);
...
void setup() {
  ...
  vslider.setLabel(F("PWM\n"));
  vslider.setValue(0); // 0 to 255
  gauge.setValue(vslider.getValue());
...
void loop() {
  ...
  if (cmd != 0) { // have parsed a complete msg { to }
    ...
    if ('.' == cmd) {
      ...
      // now handle commands returned from button/vsliders
      ...
      if (vslider.getCmd() == dwgCmd) { // user touched LED control
        int row = parser.getTouchedRow(); // row -ve for vertical slider with zero at the bottom
        vslider.setValue(-row); // set new value
        gauge.setValue(vslider.getValue()); // set gauge from vslider
        analogWrite(PWM_pin, vslider.getValue()); // set PWM
      }
...
void sendDrawing_z() {
  dwgs.start(50, 50, dwgs.WHITE); // background defaults to WHITE if omitted i.e. dwgs.start(50,35);
  parser.sendVersion(); // send the parser version to cache this image
  dwgs.pushZero(35, 25, 1.5); // move zero to centre of dwg to 25,12 and scale by 1.5 times
  gauge.draw(); // draw the control
  dwgs.popZero();
  dwgs.pushZero(15, 35); // move zero to centre of dwg to 9,30 and scale is 1 (default)
  vslider.draw(); // draw the control
  dwgs.popZero();
  dwgs.end();
}
...
void sendDrawingUpdates_z() {
  ...
  vslider.update(); // update with current state
  dwgs.end();
}
