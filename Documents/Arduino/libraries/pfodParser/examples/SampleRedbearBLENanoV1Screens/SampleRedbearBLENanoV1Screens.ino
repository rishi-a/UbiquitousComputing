/*
  Sample Pfod Screens for Redbear BLE Nano
*/
// Using RedBearLabs BLE Nano V1.5 with BLEPeripheral library V0.5.0
...
pfodParser parser("N1"); // create a parser to handle the pfod messages
...
void loop() {
  ...
  if (cmd != 0) {  // have parsed a complete msg { to }
    ...
    if ('.' == cmd) {
      ...
      if (slider.getCmd() == dwgCmd) { // user touched LED control
        ...
        analogWrite(LED_pin, 255-slider.getValue()); // set LED PWM 0 == full ON
      }
  ...
  if (millis() - rawDataTimer > RAW_DATA_INTERVAL) { // send raw data
    ...
    parser.print(','); // field separator
    parser.print(analogRead(A3)); // third field
...
void sendMainMenu() {
  ...
  parser.print(F("{,~Redbear BLE Nano\nSample Screens"));
...
void sendPlottingScreen() {
  parser.print(F("{=" // streaming raw data screen
                 "counter and ADC A3 versus time" // plot title
                 "|Time (sec)|counter|A3}")); // show data in a plot
