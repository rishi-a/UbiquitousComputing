...
def run_model(model, rgbs, N, sw):
    ...
    H_, W_ = 360, 640 # Height reduced by factor of 4, width reduced by a factor of approx 3
    #H_, W_ = 100, 100
...
def main():
    ...
    B = 1
    S = 8
    N = 20**2 # number of points to track

    filenames = glob.glob('./demo_images/P11/2/*.jpg')
    filenames.sort(key=lambda f: int(re.sub('\D', '', f)))
    #filenames = sorted(filenames)
    print('filenames', filenames)
    max_iters = len(filenames)//S # run each unique subsequence

    log_freq = 40 # when to produce visualizations 
    
    ## autogen a name
    model_name = "%02d_%d_%d" % (B, S, N)
    model_name += "_%s" % exp_name
    import datetime
    model_date = "someday" # rewritten because windows throws error in the line written below
    ...
    # create an empty tensor so that we can store the trajectories
    traj_hist = torch.empty((1,S,3,N,2))
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if global_step == 1:
                    ...
                    traj_hist = torch.cat((traj_hist,trajs_e), dim=1)

            iter_time = time.time()-iter_start_time
            print('%s; step %06d/%d; rtime %.2f; itime %.2f' % (
                model_name, global_step, max_iters, read_time, iter_time))
            print("Current Shape of Traj Hist = {}\n\n".format(traj_hist.shape))
        except FileNotFoundError as e:
            print('error', e)
    torch.save(traj_hist,'savedTraje/P11/2.pt')        
    writer_t.close()
